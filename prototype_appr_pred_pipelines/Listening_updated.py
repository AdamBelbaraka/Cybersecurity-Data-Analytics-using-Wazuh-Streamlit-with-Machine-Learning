...
MANAGER_IP = "192.168.56.104"
MANAGER_URL = f"https://{MANAGER_IP}:55000"

def get_api_token():
    r = requests.post(
        f"{MANAGER_URL}/security/user/authenticate",
        auth=HTTPBasicAuth("wazuh", "wazuh"),
        verify=False
    )
...
API_BASE = f"https://{MANAGER_IP}:55000"
...
def listen_manager_logs(offset=0, limit=50):
    params = {
        "offset": offset,
        "limit": limit,
        "sort": "-timestamp"
...
def send_atomic_archive_events(events):
    ...
    requests.post(
        f"{BASE_URL}/{CUSTOM_INDEX}/_refresh",
        auth=(USERNAME, PASSWORD),
        verify=False
    )
    return resp
...
while True:
    try:
        events = listen_manager_logs(offset)

        if events:
            print(f"🟢 {len(events)} new manager events")

            for ev in reversed(events):
                prediction = run_prediction(ev)
                write_prediction(prediction)

                offset += 1

                print(
                    f"➡ ML | score={prediction['risk_score']} "
